...
class _StateCreateProduct extends State<CreateProductScreen>
  ...
  void _createProduct() async {
    if (name!.text.isEmpty) {
      Tools.showSnackBar(
          _scaffoldKey.currentState!, S.of(context).pleaseEnterProductName);
      return;
    }

    if (infoCategory.containsKey(categorySelected) == false) {
      Tools.showSnackBar(
          _scaffoldKey.currentState!, S.of(context).pleaseChooseCategory);
      return;
    }
    if (regularPrice!.text.isEmpty) {
      Tools.showSnackBar(
          _scaffoldKey.currentState!, S.of(context).pleaseAddPrice);
      return;
    }
    ...
    await Provider.of<ProductModel>(context, listen: false)
      ...
      description!.text,
        ...
        .then((onValue) {
      Tools.showSnackBar(
          _scaffoldKey.currentState!, S.of(context).productCreateReview);
        ...
        builder: (context, value, child) {
          ...
          if (value.isLoading == false) {
            if (value.categories?.isNotEmpty ?? false) {
              ...
              for (var _category in value.categories!) {
                categorys.add(_category.name);
                infoCategory[_category.name.toString()] = _category.id;
              }
            }
          }
          return SelectInfo(
  ...
  Widget _renderBody() {
    ...
    void _onSelect(List<File>? _fileImages, List<String>? _networkImages) {
      setState(() {
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            SelectInfo(
              ...
              onChanged: (_type) {
                setState(() {
                  typeTranslationSelected = _type;
                  final index = typeTranslations
                      .indexWhere((element) => element == _type);
