...
class _OrderLineItemState extends State<OrderLineItem> {
  ...
  Widget build(BuildContext context) {
    ...
    final _allowToAssignDelivery = widget.isEditing &&
...
class FindDeliveryUserWidget extends StatefulWidget {
  ...
  @override
  _FindDeliveryUserWidgetState createState() => _FindDeliveryUserWidgetState();
}
        ...
        Container(
          ...
          child: Column(
            children: [
              ...
              if (_showSearchHinText)
                Expanded(
                  child: Center(
                    child: Text(S.of(context).searchByName),
                  ),
                ),
              if (!_showEmpty && !_isLoading)
                Expanded(
                  child: ListView.builder(
                    itemBuilder: (context, index) => Container(
                      margin: const EdgeInsets.only(bottom: 15.0),
