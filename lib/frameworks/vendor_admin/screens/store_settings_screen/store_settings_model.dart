...
import '../../../../common/tools.dart' show ImageTools;
...
class StoreSettingModel extends ChangeNotifier {
  ...
  void _dataMockUp() {
    ...
    /// Setup visibility
    visibilityData['hideEmail'] = storeSettings!.storeHideEmail!;
    visibilityData['hidePhone'] = storeSettings!.storeHidePhone!;
    visibilityData['hideAddress'] = storeSettings!.storeHideAddress!;
    visibilityData['hideMap'] = storeSettings!.storeHideMap!;
    visibilityData['hideAbout'] = storeSettings!.storeHideDescription!;
    visibilityData['hidePolicy'] = storeSettings!.storeHidePolicy!;
  ...
  Future<Map<dynamic, dynamic>> prepareData() async {
    ...
    if (_storeLogo is Asset) {
      data['logo'] = await ImageTools.compressImage(_storeLogo);
    }
    if (banner['mobileBanner'] is Asset) {
      data['mobile_banner'] =
    ...
    if (banner['storeListBanner'] is Asset &&
  ...
  Future<void> takeGalleryImages() async {
    try {
      var count = 10 - banner['slider'].length;
      final list = await MultiImagePicker.pickImages(
          maxImages: count as int, enableCamera: true);
      if (list.isNotEmpty) {
  ...
  Future<void> takeBannerImage() async {
    try {
      final list = await MultiImagePicker.pickImages(
        maxImages: 1,
        enableCamera: true,
  ...
  Future<void> takeMobileBannerImage() async {
    try {
      final list = await MultiImagePicker.pickImages(
        maxImages: 1,
        enableCamera: true,
  ...
  Future<void> takeListBannerImage() async {
    try {
      final list = await MultiImagePicker.pickImages(
        maxImages: 1,
        enableCamera: true,
  ...
  Future<void> takeLogoImage() async {
    try {
      final list = await MultiImagePicker.pickImages(
        maxImages: 1,
        enableCamera: true,
