...
const kAutoDeductBalanceForPartial  = false;

class PayPartialPayment extends StatefulWidget {
  const PayPartialPayment({Key? key}) : super(key: key);

  @override
  _PayPartialPaymentState createState() => _PayPartialPaymentState();
}
...
class _PayPartialPaymentState extends State<PayPartialPayment> {
  ...
  Widget build(BuildContext context) {
    ...
    if (user == null || user.cookie == null || kAutoDeductBalanceForPartial) return const SizedBox();
        ...
        return Padding(
          ...
          child: Column(children: [
            Row(children: [
              Checkbox(
                value: isChecked,
                onChanged: disablePayment ? null : (bool? val) {
                  setState(() {
                    isChecked = val!;
                    cartModel.setWalletAmount(val == true ? value.balance : 0.0);
                  });
                },
              ),
              Text(
            ...
            if(disablePayment) WarningCurrency(defaultCurrency: defaultCurrency),
          ],),
        );
      },
    );
        ...
        return Padding(
          ...
          child: Row(
            ...
            children: <Widget>[
              ...
              Text(
                '-' +
                    PriceTools.getCurrencyFormatted(walletAmount, currencyRate,
                        currency: currency)!,
